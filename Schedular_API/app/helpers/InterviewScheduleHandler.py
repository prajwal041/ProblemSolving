...
from app.models import Interview, InterviewConflict

"""
under def is_available
...
class InterviewScheduleHandler():
    ...
    def is_available(self):
        ...
        if interview_date != None:
            ...
            validation_result = self.check_day_is_valid(interview_date) and \
                                self.check_start_and_end_time_of_slot(interview_date) and \
                                 self.check_no_of_interviews_scheduled_in_given_slot() \
                                and self.check_interview_conflict_overlap_with_slot(interview_date)

        return validation_result

    def is_interview_duration(self):
        import datetime
        print('INSIDE duration method')
        validation_result = False
        interview_date = self.interview_datetime.date()
        ...
        interview_time_diff = interview_slot_end_time - interview_slot_start_time
        if not interview_time_diff > datetime.timedelta(hours=1, minutes=0, seconds=0):
    ...
    def check_start_and_end_time_of_slot(self, interview_date):
        ...
        interview_slot_start_time = self.localize_time(interview_date, self.interview_slot.start_time)
        interview_slot_end_time = self.localize_time(interview_date, self.interview_slot.end_time)
        validation_result = interview_slot_start_time < interview_slot_end_time
...
canceled=False,
        """
        interviews = Interview.objects.filter(calendar=self.interview_slot.calendar, start_time=self.interview_datetime)
